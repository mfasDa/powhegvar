...
void RunPythia8FromROOT(const char *pwhgin = "pwgevents.lhe", const char *foutname = "Pythia8JetSpectra.root", Int_t ndeb = 1)
{
    ...
    pythia8->ReadString(Form("Beams:LHEF = %s", pwhgin));
    ...
    if(gSystem->Getenv("CONFIG_TUNE")) {
        pythia8->ReadString(Form("Tune:pp = %s", gSystem->Getenv("CONFIG_TUNE")));
    } else {
    ...
    if(gSystem->Getenv("CONFIG_PDFSET")) {
        pythia8->ReadString(Form("PDF:pSet = LHAPDF6:%s", gSystem->Getenv("CONFIG_PDFSET")));
    } 
    ...
    pythia8->ReadString(Form("Random:seed = %lu", sseed % 900000000));
    ...
    pythia->setUserHooksPtr((shared_ptr<Pythia8::UserHooks>)powhegHooks);

...
void RunPythia8_FOCAL(const char *inputfile = "pwgevents.lhe", const char *foutname = "Pythia8JetSpectra.root", Int_t ndeb = 1) {
    clock_t begin_time = clock();

    const double MBtoPB = 1e-9;
    const double kMinEtaFOCAL = 3.4;
    const double kMaxEtaFOCAL = 5.5;
    ...
    while(true) {
        ...
        for (Int_t ip = 0; ip < np; ip++) {
            ...
            //if (abs(eta) > 0.9)
            if (eta < kMinEtaFOCAL || eta > kMaxEtaFOCAL)
                continue;
            if(charge == 0) {
        ...
        for (Int_t iR = 0; iR < nR; iR++) {
            ...
            for (unsigned int i = 0; i < inclusive_jets.size(); i++) {
                ...
                if (inclusive_jets[i].eta() < kMinEtaFOCAL+Rvals[iR] || inclusive_jets[i].eta() > kMaxEtaFOCAL - Rvals[iR])
                    continue;

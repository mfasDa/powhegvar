...
class pwgeventsparser(object):

    class __HeaderParser:
    ...
    def __init__(self, filename: str):
        ...
        self.__headerparser = self.__HeaderParser()
        self.__eventinfos = pwgevents_info()

    def get_eventinfos(self) -> pwgevents_info:
        return self.__eventinfos

    def parse(self):
        self.__eventinfos.reset()
        header_open = False
        event_open = False 
        if os.path.exists(self.__filename):
            self.__eventinfos.set_fileexists(True)
        nlines = 0
        with open(self.__filename, "r") as pwgreader:
            for line in pwgreader:
                ...
                if line_trunc.startswith("<header>"):
                    ...
                    if header_open:
                        logging.debug("Start decoding header")
                        self.__headerparser.decode(self.__eventinfos)
                    header_open = False
                elif line_trunc.startswith("<event>"):
                    event_open = True
                elif line_trunc.startswith("</event>"):
                    if event_open:
                        self.__eventinfos.add_event()
                    event_open = False
                elif line_trunc.startswith("</LesHouchesEvents>"):
                    self.__eventinfos.set_closingmarker(True)
                else:
                    if header_open:
                        self.__headerparser.add_line(line_trunc)
        if nlines > 0:
            self.__eventinfos.set_nonempty(True)

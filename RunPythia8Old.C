...
class PythiaHandler {
public:
    PythiaHandler() : mEngine(), mPDFset("default"), mTune(21), mOutput(nullptr) {}
    ~PythiaHandler(){}
    ...
    void importParticles(Option_t *option){
        if (!mOutput ) return 0;
...
private:
    ...
    std::string mPDFset;
    int mTune;
    Pythia8::PowhegHooks mPowhegHooks;
    TClonesArray *mOutput;
};  

void RunPythia8Old(const char *inputfile = "pwgevents.lhe", const char *foutname = "Pythia8JetSpectra.root", Int_t ndeb = 1) {
    ...
    if(gSystem->Getenv("CONFIG_PDFSET")) {
        pythia.setPDFset(gSystem->Getenv("CONFIG_PDFSET"));    
    }
    if(gSystem->Getenv("CONFIG_TUNE")) {
        pythia.setTune(atoi(gSystem->Getenv("CONFIG_TUNE")));    
    }
...
Bool_t IsJetOverlapping(fastjet::PseudoJet jet1, fastjet::PseudoJet jet2) {
    ...
    vector<fastjet::PseudoJet> ContJet1 = jet1.constituents();
    vector<fastjet::PseudoJet> ContJet2 = jet2.constituents();
